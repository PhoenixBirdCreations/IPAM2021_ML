...
def extractData(filename):
    ...
    data = extracted_data[:,[1,2,3,4,-3]]
    label = extracted_data[:,[-2,-1]]
    return data, label
...
class ClassificationKNN:
    ...
    def load_real_dataset(self, path, fname):
        ...
        for dat in data:
            ...
            label_test.append(dat[-2]+dat[-1])
        self.xtest = np.array(xtest)
        self_label_test = np.array(label_test)
      #  self.xtest = dat[:,indices_vars]
      #  self.label_test_ns = data[:][-2]
      #  self.label_test_rem = data[:][-1]
        self.Nfeatures  = len(self.xtest[0][:]) 
        return
