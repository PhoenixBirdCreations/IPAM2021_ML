...
Both our methods outperform the specific \ac{KNN} algorithm implementation that is currently used in the \ac{LVK} low-latency infrastructure when tested on the (GstLAL-only) \ac{O2} set
...
\ac{KNN}. \tocheck{However, it might be a bit premature to state that \ac{KNN} is more flexible than \ac{RF}. A more thorough test with data sets from new observing runs and different pipelines would be required to check this fact.}
