...
where \tocheck{\sout{$\bf X$} ${\bf X}(X)$} is a vector that identifies the output of the detection pipeline and ${\bf A}_{\bf X}$ is a vector that uniquely identifies the algorithm's output for $\bf X$. 
...
The \tocheck{\sout{ground truth} true nature (ground truth)} of observed events is unknown. Therefore, the probabilities $P(\hasns|{\bf A}_{\bf X})$ and $P(\hasrem|{\bf A}_{\bf X})$ cannot be calculated from observations. However,
...
Consider a data set of $N$ simulated events \tocheck{\sout{${\bf X}'_i$, $D=\{{\bf X}'_i\otimes{\bf L}({\bf X}'_i)\}$} ${X}'_i$, $D=\{{X}'_i\otimes{\bf L}({X}'_i)\}$}, where $i=1\dots N$ and ${\bf L}$ is a map from \tocheck{\sout{${\bf X}'$} ${X}'$} to a vector space
that defines labels for the \hasns\ and \hasrem\ \tocheck{\sout{ground truths} true events}. According to Bayes' theorem, $P(\hasns|{\bf A}_{\bf X})$  can be written as 
%
\begin{equation}
...
\end{align}
%
where $\hasns_+={\bf L}[{\bf X}'(\hbox{\hasns:\true})]$ is a label that uniquely identifies elements in $D$ \tocheck{as true events} with positive \tocheck{(+)} \hasns\ \tocheck{\sout{ground truth}}, \tocheck{\sout{${\bf X}'$}${X}'$ } is an element in $D$, and ${\bf A}_{{\bf X}'}$ is the outcome of the classifier on \tocheck{\sout{${\bf X}'$} ${X}'$}. \tocheck{The label $\hasns_+$  is assigned using the values of the parameters of the events $X'$ that are injected into the matched filtering pipeline.} The probability $P(\hasrem|{\bf A}_{\bf X})$ can be approximated as:
%
\begin{align}
\label{bayes-hasrem}
...
\end{align}
where \tocheck{\sout{$\hasrem_+={\bf L}[{\bf X}'(\hbox{\hasrem:\true})]$} $\hasrem_+={\bf L}[{X}'(\hbox{\hasrem:\true})]$} is a label that uniquely identifies elements in $D$ \tocheck{as true events} with positive \tocheck{(+)} \hasrem\ \tocheck{\sout{ground truth}} and ${\bf X}''= {\bf X}'(\hbox{\hasns:\true})$.
...
for algorithm training and validation. The $D_S$ subset is used to estimate the probabilities. Following customary practice~\tocheck{\cite{split}}, throughout this paper we use a 70\% -- 30\% split for $D_R$
...
chosen as a \tocheck{\sout{two-dimensional vector space} vector in a two-dimensional slice of the vector space ${\bf A}_{{\bf X}'}\subset \mathbb{R}^n$}. This scheme allows the calculation of Eqs.~\eqref{bayes-hasns} and \eqref{bayes-hasrem} with a single
training process. In the problem at hand, there are three possible physical states. A suitable labeling is
%
\tocheck{
\begin{align}
&{\bf L}[{X}'(\hbox{\hasns:\false})]=0\,\nonumber\\
&{\bf L}[{X}'(\hbox{\hasns:\true},\hbox{\hasrem:\false})]=1\,\\
&{\bf L}[{X}'(\hbox{\hasns:\true},\hbox{\hasrem:\true})]=2\,.\nonumber
\end{align}
}
%
...
\end{align}
%
where $N_s=\hbox{dim}\,(D_S)$ \tocheck{, i.e., the number of events in $D_S$}, $N_{\hasns_+}$ is the number of events in $D_S$ with label  $\hasns_+$, and $N^+{}_{\hasns_+}(f_1+f_2)$ and $N^-{}_{\hasns_+}(f_1+f_2)$ are the number of
...
$D_S$ with labels assigned according to the $J$-th \ac{EOS}, and $\beta_J$ are Bayes' factors \tocheck{obtained and presented in ~\cite{Ghosh:2021eqv} (see Table~II, third column)}. The probabilities $P_J(I|{\bf A}_{\bf X})$ in
