...
\end{align}
%
where $\hasns_+={\bf L}[{\bf X}'(\hbox{\hasns:\true})]$ is a label that uniquely identifies elements in $D$ with positive \tocheck{(+)} \hasns\ ground truth, ${\bf X}'$ is an element in $D$, and ${\bf A}_{{\bf X}'}$ is the outcome of the classifier on ${\bf X}'$. \tocheck{The label $\hasns_+$ is given by the matched filtering pipeline,  and therefore we are assuming that the output of the pipeline is accurate.} The probability $P(\hasrem|{\bf A}_{\bf X})$ can be approximated as:
%
\begin{align}
\label{bayes-hasrem}
...
To evaluate Eqs.~\eqref{bayes-hasns} and~\eqref{bayes-hasrem} with an \ac{ML} classifier, the synthetic data set is divided into two subsets, $D=D_R\oplus D_S$\tocheck{, where the $\oplus$ sign indicates complementary data sets} . The $D_R$ subset is used
...
chosen as a two-dimensional vector space \tocheck{\sout{${\bf A}_{{\bf X}'}\subset \mathbb{R}^n$} ${\bf A}_{{\bf X}'}\subset \mathbb{R}^2$}. This scheme allows the calculation of Eqs.~\eqref{bayes-hasns} and \eqref{bayes-hasrem} with a single
