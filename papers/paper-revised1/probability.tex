...
The \tocheck{\sout{ground truth} true nature} of observed events is unknown. Therefore, the probabilities $P(\hasns|{\bf A}_{\bf X})$ and $P(\hasrem|{\bf A}_{\bf X})$ cannot be calculated from observations. However,
...
\end{align}
%
where $\hasns_+={\bf L}[{\bf X}'(\hbox{\hasns:\true})]$ is a label that uniquely identifies elements in $D$ \tocheck{as true events} with positive \tocheck{(+)} \hasns\ \tocheck{\sout{ground truth}}, ${\bf X}'$ is an element in $D$, and ${\bf A}_{{\bf X}'}$ is the outcome of the classifier on ${\bf X}'$. \tocheck{The label $\hasns_+$ is given by the matched filtering pipeline,  and therefore we are assuming that the output of the pipeline is accurate.} The probability $P(\hasrem|{\bf A}_{\bf X})$ can be approximated as:
%
\begin{align}
\label{bayes-hasrem}
...
\end{align}
where $\hasrem_+={\bf L}[{\bf X}'(\hbox{\hasrem:\true})]$ is a label that uniquely identifies elements in $D$ \tocheck{as true events} with positive \tocheck{(+)} \hasrem\ \tocheck{\sout{ground truth}} and ${\bf X}''= {\bf X}'(\hbox{\hasns:\true})$.
