...
day and is expected to further increase in the \ac{O5}~\cite{KAGRA:2013rdx}. Over the course of \ac{O4} and the next observing runs, \tocheck{the LVK collaboration will analyze hundreds of binary black hole (BBH) candidate detections, as well as dozens of binary neutron star (BNS) and neutron star and black hole (NSBH) merger events that could potentially be MMA sources.} Among the challenges that this new phase of \ac{GW} astronomy brings is the necessity
...
\tocheck{\sout{\emph{scores}} \emph{algorithmic probabilities}}~\cite{Chatterjee:2019avs}. The \ac{KNN} model is trained on a broad set of synthetic \ac{CBC} signals injected in real detector noise from the \ac{O2}. The advantage of this
...
In this work, we revisit the problem of the real-time production of \hasns\ and \hasrem\ \emph{probability} metrics with the aim of further improving the latency and performance of the
current \ac{LVK} \ac{ML}-based scheme~\cite{Chatterjee:2019avs}. \tocheck{In the current \ac{LVK} implementation, the validity of the classification outcome for an event is assessed through elements of the
algorithm's confusion matrix and its \ac{ROC} curve, which is typically used to evaluate an algorithm's performance by plotting the \ac{TPR} against the \ac{FPR} at various threshold values. However, there is no element in the algorithm's confusion matrix that can be used to define a probability for a single event in the strictest sense. Currently, the \ac{LVK} defines the \emph{algorithmic probabilities} for \hasns\ and and \hasrem\ given by the \ac{KNN} model as the fraction of neighbors from the training test around the event in consideration. Nevertheless, this fraction is not strictly a probability. We use these fractions to build more realistic probabilities with our testing set, based on these \tocheck{algorithmic probabilities}.} A second limitation of the current \hasns\ and \hasrem\ classification implementation is the fact that the \hasns\ and \hasrem\ labels are treated as independent variables. In reality, the probability of the formation of
...
violations and the absence of inversions is only checked a posteriori through \tocheck{\sout{(coarse) parameter sweeps}evaluating the performance on the parameter space (building parameter sweeps)}. One of the main purposes of this work is to go beyond the above scheme and
...
presented in Sec.~\ref{conclusions}.  \tocheck{In Appendix~\ref{app:crossval} we deeply discuss the cross-validation procedure of our algorithms, and in Appendix~\ref{app:comparison} we explicitly show a comparison between the current \ac{LVK} implementation and ours.}
