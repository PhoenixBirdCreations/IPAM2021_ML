...
scientists to explore in depth the origin and structure of \acp{BH} \tocheck{through the observation of their progenitors}~\cite{Schmidt:2020ekt,Nitz:2021zwj,Barack:2018yly}, \acp{NS}~\cite{Ruiz:2021gsv,Baiotti:2016qnr,Lasky:2015uia} and
...
\ac{BBH} and dozens of \ac{BNS} and \ac{NSBH} detections, \tocheck{\sout{several of which} and several of the latter case} could be \ac{MMA} sources. Among the challenges that this new phase of \ac{GW} astronomy brings is the necessity
...
algorithm's confusion matrix and its \ac{ROC} curve\tocheck{, which illustrates the performance of the algorithm by plotting the \ac{TPR} against the \ac{FPR} at different threshold values.} For example, given a positive \tocheck{\sout{prediction} candidate detection}, the \ac{LVK} collaborations define a ``score'' that this 	\tocheck{\sout{prediction} candidate detection} is correct as the
