...
\tocheck{\sout{\ac{GWTC3}}\ac{GWTC}} catalog~\tocheck{\sout{\cite{LIGOScientific:2021djp}}\cite{2023ApJS..267...29A}}.
...
SPIIR. It is not surprising that our classifier performs poorly on SPIIR triggers. A similar result has been reported by other investigations~\cite{Chaudhary:2023vec}. \tocheck{\sout{Because the
...
algorithms are generally portable across pipelines and data sets, accurate results critically depend on the training set's faithful representation of the observations. \tocheck{The suboptimal performance of the pipelines, especially SPIIR, can be explained by considering that we only used GstLAL to train the algorithms. The inclusion of the other pipelines in the training phase is deferred to future work, when, hopefully, more injections will be available. We believe that this will improve the results on the other pipelines.In spite of the restriction to GstLAL triggers, our implementations still perform remarkably well when applied to other pipelines.} 
...
Finally, we apply the method to derive Bayesian probabilities for the events in the \ac{LVK} \tocheck{\sout{\ac{GWTC3}}\ac{GWTC}} catalog~\tocheck{\sout{\cite{LIGOScientific:2021djp}}\cite{2021SoftX..1300658A,2023ApJS..267...29A}.}  In Table~\ref{tab:real_data_bayesian} we
report the results for some of the most significant \ac{GWTC} events, labeled with their event ID\tocheck{, and with their \hasns\ and \hasrem\ probabilities given in their \ac{GCN} Circulars\footnote{\url{https://gcn.nasa.gov}}}. The $P_M(\hasns |\bf{A_E})$ and $P_M(\hasrem |\bf{A_E})$ probabilities for GW170817 and
...
GW170817      &  &                            & $>0.99$                 & 0.99                    & $>0.99$                   & 0.99                                \\
GW190425    & $>0.99$      & $>0.99$~\cite{2019GCN.24168....1L}                         & $>0.99$                  & 0.99                    & $>0.99$                   & 0.99                          \\
GW190426        &     $>0.99$      & $>0.99$~\cite{2019GCN.24237....1L}                        & $>0.99$                   & 0.99                    & $< 0.01$             & $< 0.01$                    \\
GW190814          &   $< 0.01$ & $<0.01$~\cite{2019GCN.25324....1L}                         & 0.04                   & 0.57                   & $< 0.01$              & $< 0.01$                      \\
GW190924      &   $0.30$ & $<0.01$~\cite{2019GCN.25829....1L}                                                     & 0.01                  & 0.05                   & $< 0.01$              & $< 0.01$                       \\               
GW200115       &     $>0.99$      & $0.09$~\cite{2020GCN.26759....1L}                                                   & $>0.99$                   & 0.99                   & $< 0.01$              & $< 0.01$                           \\
\hline
\end{tabular}
\caption{Bayesian probabilities of a few significant \ac{GW} events from the \ac{GWTC} catalog. GW170817 and GW190425, GW190426 and GW200115, and GW190814 and GW190924 were determined to be \ac{BNS}, \ac{NSBH}, and \ac{BBH} mergers, respectively\tocheck{, as stated by the second and third columns.} The probability values in the table are rounded to \tocheck{\sout{three} two} decimal figures.}
