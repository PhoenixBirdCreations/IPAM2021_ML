...
\tocheck{\ac{GWTC}}~\tocheck{\cite{2023ApJS..267...29A}}.
...
SPIIR. \tocheck{\sout{It is not surprising that our classifier performs poorly on SPIIR triggers. A similar result has been reported by other investigations~\cite{Chaudhary:2023vec}. Because the
...
algorithms are generally portable across pipelines and data sets, accurate results critically depend on the training set's faithful representation of the observations. \tocheck{The pipelines’ sub optimal performance, particularly SPIIR, can be explained by the fact that we only employed GstLAL to train the algorithms. SPIIR's interesting performance can also be seen in current LVK implementation~\cite{Chaudhary:2023vec}. Unfortunately, the inclusion of the other pipelines in the training phase requires the generation of additional injections by the LVK and is deferred to a future publication. Despite the restriction on GstLAL triggers, our approaches perform exceptionally well when applied to other pipelines. We believe that an increased injection set will further improve the performance of all pipelines. } 
...
Finally, we apply the method to derive Bayesian probabilities for the events in the \ac{LVK} \tocheck{\ac{GWTC}}~\tocheck{\cite{2021SoftX..1300658A,2023ApJS..267...29A}.}  In Table~\ref{tab:real_data_bayesian} we
...
\hline
\end{tabular}
\caption{Bayesian probabilities of a few significant \ac{GW} events from the \ac{GWTC} catalog. GW170817 and GW190425, GW190426 and GW200115, and GW190814 and GW190924 were determined \tocheck{in \ac{LVK}'s follow-up investigations to be \ac{BNS}, \ac{NSBH}, and \ac{BBH} mergers, respectively. Probabilities for GW170817 are not reported as the event was observed by \ac{EM} observatories and its true nature is confirmed. The probability values in the table are rounded to two decimal figures.}}
