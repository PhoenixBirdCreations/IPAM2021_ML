...
Equations~\ref{bayes-hasns} and~\ref{bayes-hasrem} one needs to apply
 the algorithm on known events \mmt{[MMT: applied on O2, right? Marina:yes]}. 

\begin{figure*}%[h]
\includegraphics[width=0.65\linewidth]{KNN_3_eos_prob_plots}
\caption{Fits of the Bayesian probabilities as functions of the fraction of neighbors from KNN. The left panel corresponds to the probability for \hasns\ and the right panel, to \hasrem\ . The different colors denote the equations of state. All cases increase monotonically with the fraction of neighbors. \mmt{MB: do they increase monotonically? I doubt just seeing the picture}}
