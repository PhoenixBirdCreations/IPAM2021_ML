...
\caption{ROC curves for the testing dataset for \ac{RF} classfier. All 23 EoS shown in grey, in color the EoSs with minimum and maximum mass, along commonly used SLy. \mmt{[MMT: Please unify x axis and size of the plot.]}}
\label{fig:rocO2_RF}
\end{figure*}
...
probabilities defined in Section~\ref{bayesian_probs},in terms of the output of the algorithms (fractions of neighbors and fractions of trees, respectively). In order to obtain the probabilities given in Equations \ref{bayes-hasns} and \ref{bayes-hasrem} one needs to apply the algorithm on known events \mmt{[MMT: applied on O2, right?]}. 
...
performed as explained in Section~\ref{bayesian_probs}. In this figures we show
the fits for each EOS, but one can always get a single marginalized probability
applying the Bayes factors. The fitted curves are monothonic and have a
...
As stated in Section~\ref{bayesian_probs}, the fits of the probabilities
...
\begin{figure*}%[h]
\includegraphics[width=\linewidth]{KNN_parameter_sweep}
    \caption{}
\label{fig:param_sweep_KNN}
\end{figure*}

\begin{figure*}%[h]
\includegraphics[width=\linewidth]{RF_parameter_sweep}
    \caption{}
