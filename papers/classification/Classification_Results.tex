...
(GstLAL~\cite{Sachdev:2019vvd,PhysRevD.95.042001,Sachdev:2020lfd},  \tocheck{PyCBC~\cite{Nitz:2018rgo,DalCanton:2020vpm}}, SPIIR~\cite{Chu:2020pjv}, and MBTA~\cite{Adams:2015ulm}). Therefore, we present separate \ac{ROC} curves
for these pipelines. 
...
In the case of \hasns, \ac{KNN} yields a \ac{TPR} between 0.95 and 0.98 and an \ac{FPR} smaller than 0.20 for a probability threshold of 0.5 across all pipelines, with the exception of SPIIR. \tocheck{\sout{SPIIR’s poorer performance on EM property classifications with the O3 MDC set is a known effect that has been reported in other investigations~\cite{Chaudhary:2023vec}.}} \tocheck{It is not surprising that our classifier performs poorly on SPIIR triggers. A similar result has been reported by other investigations~\cite{Chaudhary:2023vec}. Because the algorithm was not trained on multiple pipelines, a decrease in performance on triggers recovered by pipelines other than the training pipeline is to be expected. Even though the \ac{ML} algorithms are generally portable across pipelines and data sets, accurate results critically depend on the training set's faithful representation of the observations.}
In regard to \hasrem,
...
Finally, we apply the method to derive Bayesian probabilities for the events in the \ac{LVK} \ac{GWTC3} catalog~\cite{LIGOScientific:2021djp}.  In Table~\ref{tab:real_data_bayesian} we report the
