...
have established \ac{MMA} as a powerful tool for the exploration of the cosmos \todo{add refs}. The third \ac{LVK} catalog of transient \ac{GW} signals \ac{GWTC3} \todo{add ref}
...
measure the evolution of the universe \todo{add references here?}. 

Improved \ac{GW} detector sensitivities will bring a wealth of new detections to achieve these science goals \todo{ref about observing scenario and detectors}. The rate
of detections in \ac{LVK}'s \ac{O4} is expected to be more than one per day, further increasing in the \ac{O5} \todo{ref about observing scenario}. Over the course of
...
dimensionless aligned/anti-aligned spins of the objects along the orbital angular momentum. These parameters can be used to determine the \emph{\ac{EM} properties} of
...
remnant, \hasrem. Alerts with similar content and will continue to be issued in \ac{O4} with a latency of the order of one second after the detection of candidate merger events.  Additionally, \ac{LVK}'s \ac{O4} alerts will include a measure of the \hasgap\ property, i.e., the likelihood that one of the source compact objects has mass in the lower-mass gap region between \ac{NS}s and \ac{BH}s
...
The structure of the paper is as follows. In Sect.~\ref{dataset} we describe the data set and the classification schema. In Sect.~\ref{algos}, we introduce the
classification algorithms. Section \ref{results} is dedicated to the results of each method and their comparison. Conclusions and future developments are presented in
