...
\ac{BBH} and dozens of \ac{BNS} and \ac{NSBH} detections, several of which could be \ac{MMA} sources. Among the challenges that this new phase of \ac{GW} astronomy brings is the necessity
...
approach taken by the \ac{LVK} so far has been to use a specific implementation of a supervised KNeighborClassifier (\ac{KNN}) machine learning (\ac{ML})
...
algorithm's confusion matrix and its \ac{ROC} curve. For example, given a positive prediction, the \ac{LVK} collaborations define a ``score'' that this prediction is correct as the
