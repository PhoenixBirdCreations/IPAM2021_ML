...
relativity \todo{ref needed here}, probe the fundamental nature of gravity \todo{ref needed here}, and measure the evolution of the
...
One of the most interesting areas of study in \ac{GW} astronomy is the physics of gravity-matter interaction in \ac{MMA} sources. Tidally disrupted matter in an \ac{NSBH} system may form a
...
the potential of a \ac{GW} source to become an \ac{EM} emitter and enabling coincident observations of these systems by \ac{EM} and \ac{GW} observatories in real time are crucial for the success
of \ac{MMA}.
...
objects along the orbital angular momentum. These parameters can be used to determine the \emph{\ac{EM} properties} of \ac{GW} candidates in low latency through empirical fits of \ac{NR}
...
Classification of \ac{GW} candidate events in real time poses several challenges as the desire for accuracy contrasts with the desire to issue the information as quickly as possible. The
...
confusion matrix and its \ac{ROC} curve. For example, given a positive prediction, the \ac{LVK} defines the ``probability'' that this prediction is correct as the \ac{TPR} for the given data
...
Sect.~\ref{probability}. The data set is described in Sect.~\ref{dataset}. Results and algorithm comparison are reported in Sect.~\ref{results}. Conclusions and future developments are presented
