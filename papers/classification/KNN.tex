...
where $n$ is the number of features of each data point. According to usual practice, we cross-validate the prediction by computing the \mmt{accuracy}, \marco{defined as the
fraction of correctly classified events, over a range of possible values for the parameters of the algorithm.} \todo{Is this true? So is the ``score'' here the accuracy = (TPR+TNR)/N? If that is true, can we call it accuracy rather than ``score''?} \mmt{Yes, it is the mean accuracy. I've changed it.}
