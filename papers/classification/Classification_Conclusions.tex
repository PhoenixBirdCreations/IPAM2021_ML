...
may explain the discrepancy in the results]}  The decrease in performance for \hasrem\ between \ac{O2} and \ac{O3} of both algorithms may be attributed to \ac{RF} \todo{\dots} The lower performance of \ac{RF} across different pipelines compared to \ac{KNN} may be explained by \mmt{the more accurate pattern recognition of the algorithm, since each pipeline recovers the event parameters differently. While \ac{KNN} captures then main different between events, \ac{RF} learns from the specific recovery of parameters of gstLAL.}
