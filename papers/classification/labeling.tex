...
is less than the maximum \ac{NS} mass allowed by the \ac{EOS}. The value of the maximum \ac{NS} mass ranges from \tocheck{X} to \tocheck{Z} across the various \ac{EOS}, and is equal to
$\tocheck{X}M_\odot$, $\tocheck{Y}M_\odot$, and $\tocheck{Z}M_\odot$, for the \tocheck{A, B, and C} \ac{EOS}, respectively \mmt{[MMT: The EOS to consider are BHF BB2, SLy and MS1 PP, but I
don't recall the maximum masses right now. ]}. We set the \hasrem\ label as \hasrem:\true\ and \hasrem:\false\ for \ac{BNS} and \ac{BBH} systems, respectively. The value of the \hasrem\
