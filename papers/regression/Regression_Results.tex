...
\begin{figure}
    ...
    \caption{The left panel shows the injected (teal), recovered (gray), and 
            regressed (yellow) masses while the right panel shows the spins.
            }
...
\begin{figure*}[t]
    ...
    \caption{The first two panels show the recovered (gray) and regressed
             masses as a function of the injected values. The last two panels
             show the same functions for the spin magnitudes. The black line
             denotes a perfect recovery and regression. Regressed values from
             GPR are in orange while those from NN are in blue.
            }
        \label{GPRvsNN}
\end{figure*}

\lorena{Using GPR and NN to create a map between the injected and recovered
masses and spins of the O2 dataset allows us to better estimate the instrinsic 
parameters of a binary system. The advantage of using regression can be seen in
Fig~\ref{GPRvsNN}, which shows the predictions from the O2
...
\lorena{Fig~\ref{GPRvsNN} show how the primary mass is sometimes
...
recovered value tends to be more often understimated. Regression is able to
...
count$=0$ horizontal line. Table~\ref{tab:errors} quantify this errors. Greater
