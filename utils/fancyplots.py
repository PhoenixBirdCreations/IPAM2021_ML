...
def plotInjRecPred(injected, recovered, predicted, idx_m1=0, idx_m2=1, idx_Mc=None, hide_recovered=False, show=True, save=False, figname='injrecpred_'):
    ...
    if not hide_recovered:
        plt.scatter(m1_rec, m2_rec,   label='recovered', color=[0.2,0.4,1], s=3)
    plt.scatter(m1_inj, m2_inj,   label='injected', marker='x', color=[1,0.5,0], s=3)
    plt.scatter(m1_pred, m2_pred, label='predicted', color=[0,0.8,0.2], s=3)
    plt.xlabel('m1')
    plt.ylabel('m2')
    plt.legend()
    ...
    plt.figure(figsize=(4,5))
    if not hide_recovered:
        plt.scatter(m1_rec, m2_rec/m1_rec, label='recovered', color=[0.2,0.4,1], s=3)
    plt.scatter(m1_inj, m2_inj/m1_inj, label='injected', color=[1,0.5,0], s=3)
    plt.scatter(m1_pred, q_comp, label='predicted', color=[0,0.8,0.2], s=3)
    plt.xlabel('m1')
    plt.ylabel('q')
    plt.legend()
